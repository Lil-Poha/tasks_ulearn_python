"""
Увидев какой некрасивый текст находится внутри ячеек, Васе дали задание убрать весь мусор из строк и реализовать хранение
данных в более читаемой сущности.

Ему нужно дописать решение прошлой задачи. Необходимо преобразовать каждый вложенный список в словарь,
где ключом является название столбца csv, а значением пересечение столбца с текущей строкой.

Дополнительные требования:

    Если значение содержит \n (перенос строки), то его нужно разбить на список
    У всех значений (внутри вложенных списков тоже) должны удаляться пробелы (в начале, в конце строки и лишние по середине).
    А так же они должны быть очищены от HTML тегов.
